...
export async function POST(request: NextRequest) {
    try {
        ...
        // Check if there's already a FEEDBACK appeal for this order
        // Filter by type = 'feedback' OR type is null (for legacy records)
        const { data: existing } = await supabase
            .from('feedback_appeals')
            .select('id, status, type, screenshot_url')
            .eq('order_id', orderId)
        ...
        const screenshotPath = `feedback-appeals/${orderId}/proof-${timestamp}.${screenshot.name.split('.').pop()}`;
        ...
        // Create or update feedback appeal record
        if (existing) {
        ...
        console.error('Feedback appeal error:', error);
        return NextResponse.json(
