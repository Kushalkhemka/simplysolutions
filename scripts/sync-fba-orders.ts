/**
 * Sync FBA (Amazon Fulfilled) Orders from Amazon SP-API
...
async function fetchFBAOrders(accessToken: string, createdAfter: string): Promise<any[]> {
    ...
    url.searchParams.set('FulfillmentChannels', 'AFN'); // Amazon Fulfilled only
...
// Main sync function
export async function syncFBAOrders(): Promise<SyncResult> {
    ...
    try {
        ...
        // Fetch orders from last 30 days for FBA
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const createdAfter = thirtyDaysAgo.toISOString();

        const orders = await fetchFBAOrders(accessToken, createdAfter);
...
if (require.main === module) {
    ...
    console.log('=== FBA Orders Sync ===\n');

    syncFBAOrders().then(result => {
